...
Package layout (dependency arrows point only downward — no cycles):

```text
org.jwcarman.continuum        construction & client surface: Continuum,
                              DefaultContinuum, ContinuumClient,
                              RetryableContinuumClient, configs/customizers
org.jwcarman.continuum.api    the value vocabulary (ids, kinds, Outcome,
                              Computation, requests/results, typed outcomes,
                              pump parameters BatchSize/Lease/Backoff/ResultTtl)
org.jwcarman.continuum.retry  Retry, RetryConfig/Customizer, RetryContext
org.jwcarman.continuum.spi    ContinuumRepository + SPI records
```
